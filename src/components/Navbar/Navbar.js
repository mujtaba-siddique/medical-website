...
const TopBar = () => {
  const isMobile = useMediaQuery('(max-width:767px)'); // Updated breakpoint
...
const MainNavbar = ({ activeSection }) => {
  const [mobileOpen, setMobileOpen] = useState(false);
  const [chatOpen, setChatOpen] = useState(false);
  const isMobile = useMediaQuery('(max-width:1024px)'); // Updated breakpoint
